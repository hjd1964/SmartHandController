...
void UI::menuGotoSpeed() {
  ...
  char mr[20] = ""; boolean ok = onStepLx200.GetTrim(":GX93#",mr) == CR_VALUE_GET;
  if (ok) {
    double rateDefault = atof(mr);
    ok = onStepLx200.GetTrim(":GX92#",mr) == CR_VALUE_GET;
    if (ok) {
  ...
  while (current_selection_L3 != 0)
  {
    ...
    {
    case 1: onStepLx200.Set(":SX93,1#"); message.show(L_MOUNT_SPEED, "2X", 1500); break;
    case 2: onStepLx200.Set(":SX93,2#"); message.show(L_MOUNT_SPEED, "1.5X", 1500); break;
    case 3: onStepLx200.Set(":SX93,3#"); message.show(L_MOUNT_SPEED, "1X", 1500); break;
    case 4: onStepLx200.Set(":SX93,4#"); message.show(L_MOUNT_SPEED, "0.75X", 1500); break;
    case 5: onStepLx200.Set(":SX93,5#"); message.show(L_MOUNT_SPEED, "0.5X", 1500); break;
    }
  }
}
...
void UI::menuMeridianE() {
  char out[20] = "";
  if (message.show(onStepLx200.Get(":GXE9#", out))) {
    float angle = (float)strtol(&out[0], NULL, 10);
...
void UI::menuMeridianW() {
  char out[20] = "";
  if (message.show(onStepLx200.Get(":GXEA#", out))) {
    float angle = (float)strtol(&out[0], NULL, 10);
    angle = round((angle * 15.0) / 60.0);
